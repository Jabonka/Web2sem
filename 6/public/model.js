...
async function getModels() {
    let response = await fetch('http://127.0.0.1:3000/api/v3/models')
...
async function sendName(){
    ...
    if(inputName){
    let response = await fetch('http://127.0.0.1:3000/api/v3/login', {method: 'POST',headers:{"Content-type":"application/json"}, body: JSON.stringify(name_user)})
...
document.getElementById("table").addEventListener('click',function(event) {
    ...
    if(Api) {fetch(`http://127.0.0.1:3000/api/v3/models/${id}`, {method: 'DELETE', headers: {"api_key":`${Api}`}}).then(updTable)}
...
document.getElementById("add").addEventListener('click',()=>{
    ...
    <li><input id="color" value="#b062da" type="color"></li>
...
document.getElementById("addMod").addEventListener('click',async (event)=>{
    if(event.target.id !== "submit") return 0
        ...
        await fetch('http://127.0.0.1:3000/api/v3/models', {
                method: 'POST',
...
document.getElementById("table").addEventListener('click',async function(event) {
    if(event.target.id==="submit1"){
        ...
        await fetch(`http://127.0.0.1:3000/api/v3/models/${event.target.name}`, {
            method: 'PUT',
            headers: {"api_key": `${Api}`, "Content-type": "application/json"},
